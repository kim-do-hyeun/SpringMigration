...
public class DefaultStudentService implements StudentService {
    ...
    public Collection<Student> getPassedStudents() {
        ...
        return studentRepository.findAll().stream().filter(x -> x.getScore() > 60).collect(
            Collectors.toList());
    }
    ...
    public Collection<Student> getStudentsOrderByScore() {
        ...
        return studentRepository.findAll().stream().sorted(Comparator.comparing(Student::getScore).reversed()).collect(
            Collectors.toList());
    }
